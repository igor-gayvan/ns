...
def validate_and_format(df, expected_schema=None):
    ...
    try:
        ...
        if expected_schema:
            ...
            if extra_columns:
                logging.warning(f"Extra columns in the dataframe: {', '.join(extra_columns)}")

            # Data Type Validation
            for field in expected_schema:
                assert isinstance(field, StructField), "Each field in the expected schema should be of type StructField"
                actual_dtype = df.schema[field.name].dataType
                expected_dtype = field.dataType
                if actual_dtype != expected_dtype:
                    raise TypeError(f"Column '{field.name}' has datatype {actual_dtype}, but {expected_dtype} was expected")
