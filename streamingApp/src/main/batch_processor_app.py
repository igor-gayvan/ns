...
class BatchProcessorApp:
    ...
    def __init__(self, config_path='configs/application.conf', log_path='logs/batch_processor_app.log'):
        ...
        self.raw_zone_path = self.conf.get_string('s3.raw-zone-path', None)
        self.processed_zone_path = self.conf.get_string('s3.processed-zone-path', None)
    ...
    def raw_to_processed(self):
        ...
        try:
            ...
            # Reading specific partition from S3 RAW zone corresponding to today's date
            ...
            processed_df.write.option("partitionOverwriteMode", "dynamic").parquet(self.processed_zone_path,
    ...
    def validate_config_paths(self):
        ...
        if not (fs.exists(self.spark._jvm.org.apache.hadoop.fs.Path(self.raw_zone_path)) and fs.exists(
